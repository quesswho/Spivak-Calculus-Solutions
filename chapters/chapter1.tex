...
%	We define $\max(x,y)$ and $\min(x,y)$ as
%	\[
%		\max(x,y) = \begin{cases}
%			x \hspace{1cm} &\text{if $x\geq y$} \\
%			y &\text{if $y>x$}
%		\end{cases}
%	\]\[
%		\min(x,y) = \begin{cases}
%			x \hspace{1cm} &\text{if $x\leq y$} \\
%			y &\text{if $y<x$}
%		\end{cases}
%	\]
		
%		\left\{ max(x,y)=x \hspace{1cm} &\text{if $x>y$}	\right\}
...
\begin{problem} ~\\
	...
	\item ($\Rightarrow$) Suppose $-b\leq a \leq b$, this implies $a \leq b$ and $-b\leq a \iff -a \leq b$ and consequently $|a| \leq b$. \\($\Leftarrow$) Suppose $|a| \leq b$ then $a \leq b$ and $-a \leq b \iff -b \leq a$, thus $-b \leq a \leq b$. Using this fact and $|a|\geq a$ we can say that ..
